...
除了向文件中读取和写入数据，或是通过 IPC (Inter-Process Communication)，还有很多种和方法可以实现两个进程之间相互通信，但这些方法只能实现同一台机器上的进程之间的相互通信。

作为更为广泛的应用，进程一般需要通过网络，和其他机器(服务器)上的进程通信。这不会改变进程可以本地通信的事实，更重要的是，它可以跨网络进行数据交换。

<Tip>

亚马逊 CEO，Jeff Bezos 在 2000 年代初要求亚马逊服务必须通过网络公开 API。 这被认为是将亚马逊从一个简单的书店转变为 AWS 的云巨头。 这种模式现在为各地的科技公司所接受，允许团队以前所未有的速度访问数据和创新。

</Tip>

协议是为了能实现两端通信而被标准化过的一种格式。若通信时没有任何协议参与，信息将不可避免地错读或是不被理解。遵循行业标准总是比从头开始创建协议要好得多。最好在组织内采用较少数量的服务间协议，以减少实现工作量和 API 文档。

OSI 模型是一种用于描述网络协议的不同层级之间的关系的概念。官方的说，有七层，本章中会有一些例子，通常一个现代应用会需要多个层来描述构建。如下表，便于理解接下来的概念。本书主要讨论第四层，第七层，在理论上层面谈论第八层。

<table>
  <thead>
    <tr>
      <th colspan="3">Table 2-1. OSI七层模型</th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
第七层的 HTTP 协议是基于文字的协议，构建于第四层的 TCP 协议之上，是需要交付保证时的首选协议。该协议基于一个由客户端发起的 HTTP 会话请求，和一个由服务器返回的响应。该协议为浏览器而设计，用于请求网页。多年之后，它有了很多改进。增加了语义来处理压缩、缓存、错误或是重试。尽管它并不是专门为 api 而设计的，但它显然是最受欢迎的实现网络通信的交付保证协议，也是用于构建其他协议的最受欢迎的协议。

HTTP 是一个用于传输超文本的协议，比如图像或是 HTML 文档。这包括由人发现和导航的内容，不一定是应用程序代码。这一缺点将在后几个小节谈及。

有很多原因，为何 HTTP 是默认协议，用于构建面向公众 api。大多数公司已经拥有网页，故 http 的基础设施已经存在。浏览器通常需要消化这种 apis，只有少数几种协议可供浏览器使用。测试一个 api 节点，可以通过在浏览器中实际访问该 url，浏览器被广泛安装于所有开发者的机器。

### HTTP 有效载荷

HTTP，作为一个基于文字的协议，允许通信发生在无论何种平台、通过任何语言，都能通过 TCP 来通信。这也允许我能将 http 的内容写在本书中。发送一个请求，代码通常如下

```javascript:title=例子2-1使用Node.js发送HTTP请求
...
- **HTTP 方法**
  该值是在请求行的第一个单词。在例子 2.2 中，该值为 POST。其他的值还有，GET、PATCH 和 DELETE 等。这些方法对应着基础的创建读取更新删除操作，这些操作非常普遍，可以被应用于几乎所有的状态数据存储。遵守 http 方法的设计意图的应用程序，能让外部的观察者推断出某个具体请求的意图。

- **等幂性**
  多次执行某个操作而不会触发副作用，其中的 GET、PATCH 和 DELETE 都被视为等幂操作。如果某个上述操作所得到的结果是不确定的，例如，网络失败阻止了回应的接受，客户端再次发送同一请求是安全的。

- **状态码**
  在回应行中由三个数字表示。在例子 2.3 中，状态码是 403。

  ```table
  区间       类型            例子
  100-199    消息            101切换协议
  200-299    成功            200 OK, 201 创建
  300-399    重定向          301 永久重定向
  400-499    客户端错误       401 未授权, 404未找到
  500-599    服务端错误       500 服务器内部错误, 502 上游服务器响应无效
  ```

  <Note>

  状态码后面的文本称为**做原因短语(Reason Phrase)**。任何流行的 Node.js HTTP 框架都会根据应用程序指定的数字状态代码来推断要使用的文本。但现代软件未使用该值，HTTP 1.1 的后续版本 HTTP/2 不提供这样的值。

  </Note>

- **客户端错误和服务端错误**

  状态码提供了一些非常有用的信息。例如，状态范围 400-499 表示客户端出错，而状态范围 500-599 则归咎于服务器。这会通知客户端，如果尝试执行操作，并且服务器确定客户端出错，则客户端不应再次尝试发送请求。如果客户端以某种方式违反协议，就会发生这种情况。但是，当发生服务器错误时，客户端应该可以随意再次尝试幂等请求。这可能是由于服务器的临时错误，例如它被请求淹没或丢失数据库连接。在<Link to="../chapter-08/#幂等性和消息传递弹性">第 8 章 幂等性和消息传递弹性</Link>中，您将实现自定义逻辑，以根据这些状态代码重试 HTTP 请求。
...
压缩是网络负载大小和 CPU 使用率之间的权衡。通常，在 Node.js 服务器和任何使用数据的客户端之间的某个点支持 HTTP 压缩符合您的最大利益，尤其是当这是第三方通过 Internet 使用的流量时。但是，Node.js 并不是执行压缩的最有效工具。这是一个占用大量 CPU 的操作，应尽可能在 Node.js 进程之外处理。 <Link to="../chapter-03/#使用 HAProxy 的反向代理">使用 HAProxy 的反向代理</Link>着眼于使用称为反向代理的工具来自动处理 HTTP 压缩。<Link to="../chapter-03/#SLA 和负载测试">SLA 和负载测试</Link>着眼于一些基准来证明这一性能声明。
...
```javascript:title=实例2-4.server-gzip.js
...
这些 curl 命令充当通过网络与服务通信的客户端。 该服务打印请求是否使用压缩来帮助解释正在发生的事情。 在这个特定示例中，文件的压缩版本实际上比未压缩版本大！ 您可以通过运行示例 2-5 中的两个命令来看到这种情况。

```shell:title=示例2-5.比较压缩和未压缩的请求
$ curl http://localhost:1337/ | wc -c
$ curl -H 'Accept-Encoding: gzip' http://localhost:1337/ | wc -c
```

未压缩版本的文件大小位 40 字节，压缩版本的文件大小位 53 字节。

对于较大的文档，这将不是问题。 为了证明这一点，将前面的 echo 命令再运行 3 次以增加 index.html 文件的大小。 然后，再次运行示例 2-5 中的相同命令。 这次未压缩版本为 160 字节，压缩版本为 56 字节。 这是因为 gzip 通过删除响应正文中的冗余来运行，并且该示例包含重复四次的相同文本。 如果响应正文包含冗余文本（例如具有重复属性名称的 JSON 文档），这种冗余删除特别有用。 大多数 gzip 压缩工具也可以配置为当文档小于特定大小时不启用压缩。
...
接下来，把在示例 1-6 中创建的 recipe-api/producer-http-basic.js 文件中的代码复制到一个名为 recipe-api/producer-https-basic.js 的新文件中，类似于示例 2-7。 这是一个完全使用 Node.js 构建的 HTTPS 服务器。

```javascript:title=示例2-7.recipe-api/producer-https-basic.js
...
如果您要使用 Node.js 应用程序向该服务发出请求，该请求也会失败。内部 Node.js http 和 https 模块接受一个选项参数，并且 npm 中的大多数更高级别的 HTTP 库以某种方式接受这些相同的选项。避免这些错误的一种方法是提供 rejectUnauthorized: false 标志。不幸的是，这并不比使用 HTTP 更安全，应该避免。

这一切都很重要的原因是，仅仅信任互联网上遇到的任何旧证书并不一定是安全的。相反，重要的是要知道证书是有效的。这通常是通过让一个证书“签署”另一个证书来完成的。这是一种说一个证书为另一个证书做担保的方式。例如，thomashunter.name 的证书已由另一个名为 Let's Encrypt Authority X3 的证书签名。该证书已由另一个名为 IdenTrust DST Root CA X3 的证书签名。这三个证书形成了一个信任链（参见图 2-1 的可视化）。

![图2-1.证书信任链](./Figure_2-1_The_certificate_chain_of_trust.png)

链中的最高点称为根证书。该证书受到世界大部分地区的信任。事实上，它的公钥包含在现代浏览器和操作系统中。

使用自签名证书的更好方法是实际向客户端提供可信自签名证书的副本，在本例中是先前生成的 basic-certificate.cert 文件。然后可以使用 ca: certContent 选项标志传递此证书。示例 2-8 中有一个示例。

```javascript:title=示例2-8web-api/consumer-https-basic.js
...
```shell:title=示例2-9如何成为您自己的证书颁发机构
...
```graphql:title=示例 2-10 shared/graphql-schema.gql
type Query { // 顶层查询代表
  recipe(id: ID): Recipe
  pid: Int
}
...
Example 2-11. recipe-api/producer-graphql.js
...
```table:title=表2-4Common gRPC scalars
...
第 86 页的“gRPC 基准测试”包含使用 gRPC 在两台服务器之间进行通信的基准测试。

<BorderBlock>

<center>Protobufs 和 gRPC 的替代品</center>
